...
// klaus binaries, one per platform, each next to its cosign Sigstore bundle.
...
func runSelfUpdate(cmd *cobra.Command, args []string) error {
	...
	// A binary built from main rather than a tag reports its git SHA as the
	// version. Comparing that against the latest release would panic inside
	// go-selfupdate (Release.GreaterThan calls semver.MustParse), so refuse it
	// here, before anything is fetched.
	if _, err := semver.NewVersion(currentVersion); err != nil {
		return fmt.Errorf("cannot self-update: the running binary reports version %q, which is not a release version (was it built from a tag?)", currentVersion)
	}
